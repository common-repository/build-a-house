...
class iworks_options {
	...
	public function build_options( $option_group = 'index', $echo = true, $term_id = false ) {
		...
		if ( ! is_array( $options['options'] ) ) {
			echo '<div class="below-h2 error"><p><strong>' . __( 'An error occurred while getting the configuration.', 'build-a-house' ) . '</strong></p></div>';
			return;
		}
		/**
		 * proceder
		...
		if ( $options['show_submit_button'] ) {
			$content .= get_submit_button( __( 'Save Changes', 'build-a-house' ), 'primary', 'submit_button' );
		}
		/**
...
//]]>
	...
	private function location( $name, $value = '', $args = array() ) {
		...
		$i18n     = array(
			'country' => __( 'Country', 'build-a-house' ),
			'city'    => __( 'City', 'build-a-house' ),
			'street'  => __( 'Street', 'build-a-house' ),
			'zip'     => __( 'ZIP code', 'build-a-house' ),
		);
		...
		$f = array(
			...
			array(
				...
				'wp_localize_script' => array(
					'buttons' => array(
						'select_media' => __( 'Select Image', 'build-a-house' ),
					),
				),
			),
			/**
	...
	public function get_switch_button_data() {
		$data = array(
			'labels' => array(
				'off_label' => __( 'OFF', 'build-a-house' ),
				'on_label'  => __( 'ON', 'build-a-house' ),
			),
		);
		return $data;
	}
